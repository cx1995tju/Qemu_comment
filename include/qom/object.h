 ...
 * run time.
 */
/* 这个结构是用类来创建对象的关键，其中的class成员执行一个Objectclass实例，从而使用container操作找到自己之前定义的一个类
 * 其中包含了如何创建一的对象的信息，譬如：对象的大小，构造函数等。
 *
 * 所以使用一个类创建一个对象的关键就是：
 *  0. 搞一个TypeImpl的实例，其中包含一些类的元信息，创建ObjectClass实例的时候需要
 *  1. 继承自ObjectClass，创建一个MyClass（首成员必须是ObjectClass）
 *  2. 继承自Object，创建一个MyObject（首成员是MyClass中的ObjectClass成员）
 *  3. 使用MyObject类型来创建一个对象
 * 
 * 这个对象中的Object类型成员是一个Object实例，而这个Object实例中的首成员是一个ObjectClass实例，(这里我们使用container作用于OBjectClass实例的话，能够得到定义的MyClass实例)
 * 这个ObjectClass实例中又包含了一个TypeImpl实例，（这个实例就是我们使用TypeInfo实例注册得到的）
 *
 *
 * 显然MyObject创建得到的实例，其中包含了相关类的所有元信息。通过这些元信息使得该类可以被QOM管理，可以实现封装，继承，多态等机制。
 *
 * TypeInfo结构是方便我们创建新类并注册的一个保存一些信息的结构。然后调用Type_register就能得到一个TypeImpl的实例了。
 * */
struct Object
{
    /*< private >*/
 ...
 *   for initializing its own members. 对象的构造函数
 * @instance_post_init: This function is called to finish initialization of
 *   an object, after all @instance_init functions were called. 对象构造结束后调用该函数
 * @instance_finalize: This function is called during object destruction.  This
 *   is called before the parent @instance_finalize function has been called.
 *   An object should only free the members that are unique to its type in this
 *   function. 对象的析构函数
 * @abstract: If this field is true, then the class is considered abstract and
 *   cannot be directly instantiated. 抽象类么？
 * @class_size: The size of the class object (derivative of #ObjectClass)
 *   for this object.  If @class_size is 0, then the size of the class will be
 *   assumed to be the size of the parent class.  This allows a type to avoid
 *   implementing an explicit class type if they are not adding additional
 *   virtual functions. 类的大小，这里的类并不是CPP中的类，因为并不能直接根据这个类来创建对象的。可以认为这个类仅仅是CPP中类的概念的一部分。
 * @class_init: This function is called after all parent class initialization
 *   has occurred to allow a class to set its default virtual method pointers.
 *   This is also the function to use to override virtual methods from a parent
 *   class. 让类来重写其虚函数使用的。虚函数不是对象的成员的
